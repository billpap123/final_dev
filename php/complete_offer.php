...
try {
    ...
    if ($offer_id) {
        $stmt = $conn->prepare("UPDATE offers SET status = 'Completed' WHERE offer_id = ?");
        $stmt->bindParam(1, $offer_id, PDO::PARAM_INT);
        if ($stmt->execute()) {
            $stmt = $conn->prepare("UPDATE tasks SET status = 'Completed', completed_date = NOW() WHERE offer_id = ?");
            $stmt->bindParam(1, $offer_id, PDO::PARAM_INT);
            if ($stmt->execute()) {
