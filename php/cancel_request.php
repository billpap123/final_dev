...
try {
    ...
    if ($request_id) {
        $stmt = $conn->prepare("UPDATE requests SET status = 'Cancelled' WHERE request_id = ?");
        $stmt->bindParam(1, $request_id, PDO::PARAM_INT);
        if ($stmt->execute()) {
            $stmt = $conn->prepare("UPDATE tasks SET status = 'Cancelled', completed_date = NOW() WHERE request_id = ?");
            $stmt->bindParam(1, $request_id, PDO::PARAM_INT);
            if ($stmt->execute()) {
